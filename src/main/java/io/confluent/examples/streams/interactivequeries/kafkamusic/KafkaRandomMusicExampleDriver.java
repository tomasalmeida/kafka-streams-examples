 ...
 */
public class KafkaRandomMusicExampleDriver {

  private static final Locale[] countries = Locale.getAvailableLocales();
  ...
  public static void main(final String [] args) throws Exception {
    ...
    while (true) {
      ...
      final String country = countries[random.nextInt(countries.length)].getCountry();

      if (StringUtils.isNotBlank(country)) {
        System.out.println("Writing play event for song [" + song.getName() + "] to input topic [" +
                KafkaMusicExample.PLAY_EVENTS + "] with key [" + country + "]");
        playEventProducer.send(
                new ProducerRecord<>(KafkaMusicExample.PLAY_EVENTS, country, new PlayEvent(song.getId(), duration)));
        Thread.sleep(5000L);
      }
    }
  }
